...
public class CentralDirectoryEntry implements ZipConstants {
	...
	private static final Logger LOG = Logger
			.getLogger(CentralDirectoryEntry.class.getName());
	...
	protected void initFromRaFile() throws IOException {
		...
		if (censig != CENSIG) {
			throw new ZipException(
					"expected CENSIC not found in central directory (at end of zip file)");
		...
		if (LOG.isLoggable(Level.FINE)) {
			LOG.fine("CDS - extraFieldOffset ="
					+ Long.toHexString(this.extraFieldOffset));
			LOG.fine("CDS - extraFieldLength =" + this.extraFieldLength);
			LOG.fine("CDS - localHeaderOffset="
					+ Long.toHexString(this.localHeaderOffset));
		}

		// TODO - check, why we have to use the local header instead of the CDS
		// sometimes...
		...
		if (this.isEncrypted) {
			...
			if (efhid[0] != 0x01 || efhid[1] != (byte) 0x99) {
				...
				this.extraFieldLength = raFile
						.readShort(localHeaderOffset + 28);
				if (LOG.isLoggable(Level.FINE)) {
					LOG.fine("local header - extraFieldOffset="
							+ Long.toHexString(this.extraFieldOffset));
					LOG.fine("local header - extraFieldLength="
							+ Long.toHexString(this.extraFieldLength));
				}
			...
			if (this.isAesEncrypted) {
				this.actualCompressionMethod = raFile
						.readShort(getExtraFieldOffset() + 9);
				this.localHeaderSize = 30 + getExtraFieldLength()
						+ getFileNameLength();
			}
		}
	...
	public short getCryptoHeaderLength() {
		// TODO support 128+192 byte keys reduces the salt byte size to 8+2 or
		// 12+2
		return 18;
	}
